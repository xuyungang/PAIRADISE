...
def main():
  ...
  ## assa parameters
  parser.add_argument('--sampleDirList',help="The file contains sample directory.Each directory path a row. Do not include the haplotype bam file.")
  parser.add_argument('--ASASOutDir',help="The output folder of ASAS event count result.")
...
#  parser.add_argument('--exonreads',help="to include exon reads")
  ...
  if ((args.rnaedit and not args.e) or (args.e and not args.rnaedit)):
    sys.stderr.write("pairadise22: if --rnaedit flag is used, you must also provide a file containing RNA editing locations using -e parameter \n")
    sys.exit()
  if len(command)==0:
    sys.stderr.write('pairadise2: need a command - personalize, mapping, discover, alleles, splicing, assaevent, statistics\n')
    sys.exit()
  if (not args.o):
    ...
    sys.stderr.write("pairadise2: I don't recognise the option '" + command[0] +"'\n")
    sys.exit()
