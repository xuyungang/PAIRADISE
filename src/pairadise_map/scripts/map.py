...
def main(args):
  ...
  helpStr = "Help!\n"
  if not args.o:
    sys.stderr.write('pairadise22 ERROR: must provide output directory \n\n')
    sys.exit()
  if not args.s:
    sys.stderr.write('pairadise2 ERROR: must provide read sequence files \n\n')
    sys.exit()
#  if not args.g:
#    sys.stderr.write('pairadise2 ERROR: must provide gtf file\n')
  ...
  if len((args.s).split(','))==0 or len((args.s).split(','))>2:
    sys.stderr.write("ERROR: Sequence parameter -s input is  not correct\n Example: pairadise2 run mappng alleles -s reads_1.fq,reads_2.fq -o pairadise2\n")
    sys.exit()
...
#    shutil.rmtree(os.path.join(outDir, "HAP2/STARindex"))
    ...
    if not args.r:
      sys.stderr.write("ERROR: pairadise2 run mapping command requires -r parameter \nExample: pairadise2 run mapping -r reference.fa -s reads_1.fq,reads_.fq -o pairadise2 \n")
      sys.exit()
